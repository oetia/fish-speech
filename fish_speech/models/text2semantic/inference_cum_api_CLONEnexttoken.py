    ...
    **sampling_kwargs,
    ...
    for i in tqdm(range(num_new_tokens)):
        ...
        with (
            ...
            yield ("next_token", next_token.clone())
    ...
    yield ("previous_tokens", previous_tokens[:, : i + 1])
    ...
    for response in generator:
        if isinstance(response, GenerateResponse):
            if response.action == "sample":
                ...
                if codes:
                    indices = torch.cat(codes, dim=1)
                    codes_npy_path = os.path.join(output_dir, f"codes_{idx}.npy")
                    np.save(codes_npy_path, indices.cpu().numpy())
                    logger.info(f"Saved codes to {codes_npy_path}")

                    fake_audio = vqgan_inference(indices)
                    audio_path = os.path.join(output_dir, f"out.wav")
                    sf.write(audio_path, fake_audio, model_vqgan.spec_transform.sample_rate)
                    logger.info(f"Saved audio to {audio_path}")

                    # sanity check to make sure tokens and codes are the same in the end
                    print(torch.cat(tokens, dim=1).shape, indices.shape)
                    print(torch.all(torch.cat(tokens, dim=1).isclose(indices)))
                    print((torch.cat(tokens, dim=1).isclose(indices)).sum())
                    print((~torch.cat(tokens, dim=1).isclose(indices)).sum())
                    # sanity check failed. data is fundamentally different. need to check to see if after the next_token yield there's some extra stuff to do with processing
                logger.info(f"Next sample")
                codes = []
                idx += 1
            else:
                logger.error(f"Error: {response}")
        elif response[0] == "next_token":
...
if __name__ == "__main__":
    main(text="真让人怀念，很多年前，我也曾如今天这般走入那谋臣似雨的阁楼。",
