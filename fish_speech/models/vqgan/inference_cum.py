...
def main(input_path, output_path, config_name, checkpoint_path, device):
    ...
    # Restore
    def restore(indices: torch.Tensor, endpoint: int):
        print(indices.shape)
    ...
    print(indices.shape)
    endpoints = list(range(0, indices.shape[1], 10))[1:] + [indices.shape[1]]
    for idx, endpoint in enumerate(endpoints):
        print(endpoint)
        restore(indices=indices[:,:endpoint], endpoint=endpoint)
